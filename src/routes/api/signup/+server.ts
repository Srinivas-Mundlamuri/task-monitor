...
export const POST: RequestHandler = async ({ request }) => {
  ...
  const mutation = `
    mutation CreateProfile($name: String!, $password: String!) {
      insert_profiles_one(object: { name: $name, password: $password }) {
        id
        name
      }
    }
  ...
  const res = await fetch(GRAPHQL_URL, {
    method: 'POST',
    headers,
    body: JSON.stringify({ query: mutation, variables: { name, password } }),
  });

  const data = await res.json();
  if (!res.ok || data.errors) return json({ error: data.errors ?? 'unknown' }, { status: 500 });

  return json({ profile: data.data.insert_profiles_one });
};
